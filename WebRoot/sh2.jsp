...
String sql;
sql="update t_"+request.getParameter("tablename")+" set issh='"+request.getParameter("issh")+"',shhf='"+request.getParameter("shhf")+"' where id="+request.getParameter("id");
connDbBean.hsgexecute(sql);
out.print("<script>javascript:alert('审核成功！');location.href='"+request.getParameter("tablename")+"List.action';</script>");
}
...
%>
  ...
  <body >
    ...
    <tr>
      <td>审核回复：</td>
      <td height="80"><textarea name="shhf" cols="50" rows="5" id="shhf"><%=connDbBean.readzd("t_"+request.getParameter("tablename"),"shhf","id",request.getParameter("id"))%></textarea></td>
    </tr>
	<script language="javascript">
	document.form1.issh.value="<%=connDbBean.readzd("t_"+request.getParameter("tablename"),"issh","id",request.getParameter("id"))%>";
	</script>
